...
public class Database extends SQLiteOpenHelper {
    ...
    //lấy ID của chi tiết lớp học
    public int getIdStudentInCAT (int Code){
        int ClassDetailId = -1;
        SQLiteDatabase SQL = getReadableDatabase();
        Cursor cursor = SQL.rawQuery("Select * from ClassDetailsAttendance", null);
        {
            if (cursor.moveToFirst()) {
                do {
                    //if (CodeAttendance.equals(cursor.getString(8))) {
                        if (Code == cursor.getInt(2)) {
                        ClassDetailId = cursor.getInt(0);
                    }
                } while (cursor.moveToNext());
            }
        }
