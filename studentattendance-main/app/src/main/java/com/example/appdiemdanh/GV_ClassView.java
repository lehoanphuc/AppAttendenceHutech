    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnCreateAttendance.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Cursor cursorDiem = database.GetData("SELECT ClassDetailsId, AccountTable.AccountId, " +
                        "ClassTable.ClassId, ClassTable.ScorePercent, ClassTable.NumOfAttendance, Score, " +
                        "SoBuoiDaDiemDanh, ClassDetailsTable.Status FROM ((ClassDetailsTable " +
                        "INNER JOIN ClassTable ON ClassTable.ClassId = ClassDetailsTable.ClassId) " +
                        "INNER JOIN AccountTable ON AccountTable.AccountId = ClassDetailsTable.AccountId) " +
                        "WHERE ClassDetailsTable.ClassId = '" + idClass + "'");

                if(cursorDiem.moveToFirst()){
                    do{
                        if(idClass == cursorDiem.getInt(2)){
                            float LayBuoiDiemDanh = cursorDiem.getInt(4);
                            float LayDiemQuaTrinh = cursorDiem.getInt(3);
                            ...
                            int SoBuoiDaDiemDanh = cursorDiem.getInt(6);
                            float SoBuoiTinhDiem = cursorDiem.getInt(6);
                            float DiemTongSV = DiemTheoTungBuoi * SoBuoiTinhDiem;
                            int ClassDetailsId = cursorDiem.getInt(0);
...
//                database.QueryData("SELECT ClassDetailsId, AccountTable.AccountId, ClassTable.ClassId, ClassTable.ScorePercent, ClassTable.NumOfAttendance, Score, SoBuoiDaDiemDanh, ClassDetailsTable.Status\n" +
//                        "FROM ClassDetailsTable \n" +
//                        "INNER JOIN ClassTable ON ClassTable.ClassId = ClassDetailsTable.ClassId\n" +
//                        "INNER JOIN AccountTable ON AccountTable.AccountId = ClassDetailsTable.AccountId\n" +
//                        "WHERE ClassDetailsTable.ClassId =1");
